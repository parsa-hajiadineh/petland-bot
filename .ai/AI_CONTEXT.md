...
اشتراک فروشگاه مستقل از فاکتور است (`TenantSubscription`): شروع/پایان دوره، مبلغ، وضعیت، اسنپ‌شات خدمات (`servicesJson`). با تایید فاکتور خدمات پر می‌شود. وضعیت محاسبه‌شده برای هر ربات: `ACTIVE` / `EXPIRING_SOON` (۵ روز مانده) / `EXPIRED` / `SUSPENDED` / `CANCELLED`. متن یادآوری هنوز ارسال نمی‌شود؛ `listShopLifecycles()` برای تنظیم متن بعدی آماده است.
...
## کار بعدی
1. متن‌های یادآوری اشتراک (۵ روز مانده / منقضی) روی ربات همکار
2. TenantMessage از پنل ادمین مادر
3. TENANT_RESELL
