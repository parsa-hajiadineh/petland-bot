...
دکمه «🤝 خرید همکار» اول دو راه می‌دهد: ورود همکار / ورود بازاریابان مانلی. هر کدام کد env جدا دارد (`COLLEAGUE_ACCESS_CODE` / `MANELI_ACCESS_CODE`). نقش `MANELI` قیمت همکاری می‌بیند و سفارش `PL-` ثبت می‌کند، ولی ساخت ربات / اعتبار / گلدن / پکیج / کیف پول بازاریابی ندارد. فاکتور مانلی در پنل ادمین «سفارش‌های مشتریان» نوع سوم است. تایید مانلی اعتبار و پورسانت نمی‌سازد. نوتیف مانلی روی ربات مادر است. آمار فروش بخش جدا برای حجم/تعداد مانلی دارد و سودش صفر است.

کد مخفی بازاریابی برای `COLLEAGUE` و `MANELI` کار نمی‌کند و دکمه‌های بازاریابی/کیف پول در این حالت‌ها دیده نمی‌شوند. همکار و مانلی دکمه بازگشت به خرید خرد ندارند؛ فقط ادمین مادر `👤 بازگشت به خرید عادی` دارد (نقش ADMIN می‌ماند، قیمت خرد برای تست). تیکت عنوان جدا ندارد؛ لیست‌ها متن پیام را نشان می‌دهند. پنل ادمین «🔍 جستجوی تیکت» با کد `#xxxxxx`. در جزئیات تیکت نوع حساب ارسال‌کننده (یوزر / همکار) مشخص است.
...
منوی پنل ادمین مادر و پنل ادمین فروشگاه همکار: دو دکمه در هر ردیف. دکمه سفارش مادر: «🧾 سفارش‌های مشتریان»؛ اول نوع فاکتور (خرید عادی / خرید همکار / بازاریابان مانلی) بعد وضعیت. دکمه اعتبار: «⚙️ تنظیمات اعتباردهی»؛ بازگشت از ورود عدد به همان صفحه تنظیمات برمی‌گردد. «🛡 مدیریت»: جستجوی هویتی (نام/تلفن/آیدی/…) اینلاین؛ مشتری = پرونده متنی؛ همکار = پرونده کامل فروشگاه/ربات/اشتراک/اعتبار + مسدود/رفع مسدودی. مسدود یعنی `Tenant.status=SUSPENDED` و ربات `DISABLED`/`isEnabled=false` تا خود همکار و مشتریانش به ربات فروشگاه دسترسی نداشته باشند؛ وضعیت قبلی برای رفع مسدودی نگه داشته می‌شود. زیرمنوی «🚫 مسدود شدگان». دکمه پکیج مادر: «🛠 تغییر پکیج های خدماتی». پنل تننت: «⚙️ تنظیمات فروشگاه» زیرمنوی لوگو/مشخصات/خوش‌آمد/راهنما/کارت/دسته/کالا؛ «🧾 سفارش‌های مشتریان». سفارشات بسته اینلاین حداکثر ۱۰ تا + «ده سفارش قبلی».
