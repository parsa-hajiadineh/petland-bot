...
خرید اشتراک: تا آخرین فاکتور خدمات `APPROVED` یا `REJECTED` نشود اشتراک جدید صادر نمی‌شود (`WAITING_PAYMENT` / `WAITING_APPROVAL` قفل است). رد ادمین مادر از «فاکتور خدمات همکاران» با `REJECTED` (نه `approveOrder`). فاکتور بعدی همان `periodStart`/`periodEnd` فاکتور ردشده را می‌گیرد؛ `lastPeriodEnd` فقط فاکتورهای `APPROVED` است. ثبت/رسید فاکتور خدمات از ربات تننت با `notifyMother` به ادمین مادر می‌رسد. تایید/رد همان فاکتور و نوتیف افزایش اعتبار خرید همکار با `notifyShop` روی ربات فروشگاه همکار می‌رود. فاکتور در انتظار پرداخت در لیست همکار دکمه «💳 تکمیل پرداخت» دارد؛ بعد از نمایش کارت، «📸 ارسال رسید پرداخت». «💵 پرداخت نقدی» پیش‌فاکتور را از حالت اعتبار برمی‌گرداند. ادمین مادر با کلیک `sinv:` رسید را هم می‌بیند؛ رد فاکتور خدمات علت می‌پرسد.
