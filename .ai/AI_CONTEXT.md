...
## وضعیت همین الان (۱۴۰۵/۰۶/۰۶ — ادامه بده از اینجا)

**مادر کار می‌کند.** ربات تست تا این فیکس روی `/start` می‌گفت «فروشگاه الان پاسخ نداد» چون `src/handlers/tenantOrder.js` موقع `require` کرش می‌کرد:

`ReferenceError: QTY_STEP is not defined`

ثابت‌های `QTY_STEP = "TCK:QTY"` و `RECEIPT_STEP = "TCK:RECEIPT"` باید بالای فایل بمانند. بدون آن‌ها `tenantShop` لود نمی‌شود و `router.js` همان پیام خطا را نشان می‌دهد.

بعد از دپلوی: در ربات تست `/start` → باید خوش‌آمد فروشگاه بیاید. بعد دسته → کالای اینلاین ۱۰تایی → سبد → تسویه را تست کن.

اگر باز خطا بود لاگ لیارا: `TENANT SHOP:` / `TENANT MESSAGE:` / `GET UPDATES FAIL`. توکن ربات را لاگ نکن.
...
## کار بعدی
1. دپلوی این فیکس و تست کامل خرید در ربات تست
