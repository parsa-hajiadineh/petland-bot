...
async function waitSetupApproval(user, chatId, invoice) {
  ...
  await reply(
    user,
    chatId,
    `فاکتور راه‌اندازی${code} هنوز توسط ادمین پائورا تایید نشده است.
...
•توجه داشته باشید که ثبت سفارش به این شکل است که بعد از انتخاب محصولات مد نظر و صدور پیش فاکتور، کالاهای درخواستی شما با موجودی لحظه ای انبار چک می‌شود و در صورت تایید پیش فاکتور، به شما اطلاع داده می‌شود؛ لازم به ذکر است که پیش فاکتور های تایید شده تا ۳۰ دقیقه قابلیت پرداخت داشته و اعتبار دارند.
